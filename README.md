...
Welcome to our project repo.  Since this is forked from another
project here on GitHub, the bottom of this repo contains the README of the
original author at the time of forking.  You can reproduce our results (paper to
be linked) with the following command after downloading the prerequisites (see
original README):

```
$ python3 train.py \
  --pre_trained_model resnet_v2_101.ckpt
  --train_epochs 10 \
  --batch_size 6 \
  --weight_decay 2e-4 \
  --model_dir models/ba=6,wd=2e-4,max_iter=12.5k,0.005-sobel \
  --max_iter 12500 \
  --initial_learning_rate 0.01 \
  --psi_type <ZERO|ONES|GAUSSIAN|SOBEL|LANCZOS|LAPLACIAN_OF_GAUSSIAN> \
  --psi_scale 0.0005 \
  --psi_param <number>
```
