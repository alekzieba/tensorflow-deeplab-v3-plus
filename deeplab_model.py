...
def atrous_spatial_pyramid_pooling(inputs, output_stride, batch_norm_decay, is_training, psi_type=_PSI_ZERO, psi_param=1, psi_scale=1.0, depth=64):
  ...
  with tf.variable_scope("aspp"):
    ...
    atrous_rates = [6, 8, 10]  # was [6, 12, 18]
    if output_stride == 8:
    ...
    with tf.contrib.slim.arg_scope(resnet_v2.resnet_arg_scope(batch_norm_decay=batch_norm_decay)):
      with arg_scope([layers.batch_norm], is_training=is_training):
        ...
        if psi_type == _PSI_ZERO:
          ...
          filter_1 = create_lanczos_filter(psi_param, kernel_shape_1, psi_scale)
          filter_2 = create_lanczos_filter(psi_param, kernel_shape_2, psi_scale)
          filter_3 = create_lanczos_filter(psi_param, kernel_shape_3, psi_scale)
